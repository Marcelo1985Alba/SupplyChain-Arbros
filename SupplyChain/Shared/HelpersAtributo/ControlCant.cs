    ...
    protected override ValidationResult IsValid(object value, ValidationContext validationContext)
    {
        ...
        if ((stock.TIPOO == 10 || stock.TIPOO == 27) && stock.STOCK > stock.ResumenStock?.STOCK) //entrega con y sin of
            return new ValidationResult(
                $"{stock.CG_ART.Trim()}: No se pueden entregar cantidades mayores al de stock.");

        if ((stock.TIPOO == 21 || stock.TIPOO == 27 || stock.TIPOO == 10 || stock.TIPOO == 28) &&
            stock.STOCK == 0) //ajuste inventario entrega con y sin of
