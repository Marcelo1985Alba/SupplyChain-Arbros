...
public class ProductosPageBase : ComponentBase
{
        ...
        if (args.Item.Id == "Copy")
            ...
            if ((await refGrid.GetSelectedRecordsAsync()).Count > 0)
            {
                var isConfirmed = await jSRuntime.InvokeAsync<bool>("confirm",
                    "Seguro de que desea eliminar los productos seleccionados?");
                if (isConfirmed)
                {
                    var productosABorrar = await refGrid.GetSelectedRecordsAsync();
    ...
    private async Task CopiarProducto()
    {
        if (refGrid.SelectedRecords.Count == 1)
        {
            ...
            var isConfirmed = await jSRuntime.InvokeAsync<bool>("confirm", "Seguro de que desea copiar el producto?");
            if (isConfirmed)
            {
