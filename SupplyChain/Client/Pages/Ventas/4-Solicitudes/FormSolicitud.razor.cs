...
public class FormSolicitudBase : ComponentBase
{
    ...
    protected PreciosArticulos PreciosArticulos = new();
    ...
    protected bool popupBuscadorVisibleFormPrecio { get; set; }
    ...
    protected async Task Guardar()
    {
        ...
        if (guardado)
        {
            if (Solicitud.CalcId > 0) await SolicitudService.Imprimir(Solicitud.Id);
    ...
    {
        PreciosArticulos = new PreciosArticulos();
        if (!string.IsNullOrEmpty(Solicitud.Producto))
        {
            ...
            {
                if (response.Response != null) PreciosArticulos = response.Response;
    ...
    {
        if (precio.GUARDADO) Solicitud.Des_Prod = precio.Descripcion;
