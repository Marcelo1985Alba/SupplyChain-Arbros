...
public class ServicioPageBase : ComponentBase
{
    ...
    protected List<Cliente> ClienteList = new();
    protected DialogSettings DialogParams = new() { MinHeight = "400px", Width = "1100px" };
    ...
    protected List<Service> servicios = new();
    ...
    public async Task ActionBegin(ActionEventArgs<Service> args)
    {
        if (args.RequestType == Action.Save)
        {
            ...
            var found = servicios.Any(o => o.Id == args.Data.Id);
            var ur = new Service();
            ...
            else
            {
                    ...
                    foreach (var ped in servicios)
                        if (args.Data.PEDIDOANT == ped.PEDIDO.ToString())
                        {
                            var isConfirmed = await JsRuntime.InvokeAsync<bool>("confirm",
                                "Quiere traer los datos del pedido anterior?");
                            if (isConfirmed)
                            {
                ...
                args.Data.CAMBIOREPUESTO =
                    string.IsNullOrEmpty(args.Data.CAMBIOREPUESTO) ? "" : args.Data.CAMBIOREPUESTO;
                ...
                args.Data.PRESENCIAINSPEC =
                    string.IsNullOrEmpty(args.Data.PRESENCIAINSPEC) ? "" : args.Data.PRESENCIAINSPEC;
    ...
    private async Task EliminarServicio(ActionEventArgs<Service> args)
    {
        ...
        {
            var isConfirmed = await JsRuntime.InvokeAsync<bool>("confirm",
                $"Seguro de que desea eliminar la reparacion {args.Data.Id}?");
            if (isConfirmed)
            {
    ...
    public async Task ClickHandler(ClickEventArgs args)
    {
        ...
        if (args.Item.Text == "Exportar grilla en Excel") await Grid.ExcelExport();
        if (args.Item.Text == "Certificado")
            if (Grid.SelectedRecords.Count > 0)
                foreach (var selectedRecord in Grid.SelectedRecords)
                {
                    ...
                    //Add RowSpan
                    var gridCell = pdfGrid.Rows[0].Cells[0];
                    ...
                    gridCell2.Value = new PdfTextElement("CERTIFICADO DE MANTENIMIENTO Y CALIBRACIÓN", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Centrado);
                    //Add RowSpan
                    var gridCell3 = pdfGrid.Rows[1].Cells[2];
                    gridCell3.ColumnSpan = 2;
                    gridCell3.StringFormat = Centrado;
                    gridCell3.Value = new PdfTextElement("VALVULA DE SEGURIDAD Y ALIVIO", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Centrado);
                    //Add RowSpan
                    var gridCell4 = pdfGrid.Rows[0].Cells[4];
                    gridCell4.ColumnSpan = 2;
                    gridCell4.StringFormat = Centrado;
                    gridCell4.Value =
                        new PdfTextElement(
                            $"FECHA: {DateTime.Today.Day} / {DateTime.Today.Month} / {DateTime.Today.Year}", font,
                            new PdfPen(PdfColor.Empty), PdfBrushes.Black, Centrado);
                    //Add RowSpan
                    var gridCell5 = pdfGrid.Rows[1].Cells[4];
                    gridCell5.ColumnSpan = 2;
                    gridCell5.StringFormat = Centrado;
                    gridCell5.Value = new PdfTextElement($"PEDIDO: {selectedRecord.Id}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Centrado);
                    //Add RowSpan
                    var gridCell6 = pdfGrid.Rows[2].Cells[0];
                    gridCell6.ColumnSpan = 6;
                    gridCell6.StringFormat = Centrado;
                    gridCell6.Value =
                        new PdfTextElement(
                            "ENSAYOS EFECTUADOS EN BANCO CON PULMÓN HIDRO NEUMÁTICO. FLUIDO DE PRUEBA: AIRE A TEMPERATURA AMBIENTE",
                            font, new PdfPen(PdfColor.Empty), PdfBrushes.Black, Centrado);
                    //Add RowSpan
                    var gridCell7 = pdfGrid.Rows[3].Cells[0];
                    gridCell7.ColumnSpan = 6;
                    gridCell7.StringFormat = Centrado;
                    gridCell7.Value = new PdfTextElement("GENERALIDADES", font, new PdfPen(PdfColor.Empty),
                        PdfBrushes.Black, Centrado);
                    //Add RowSpan
                    var gridCell8 = pdfGrid.Rows[4].Cells[0];
                    gridCell8.ColumnSpan = 3;
                    gridCell8.StringFormat = Izquierda;
                    gridCell8.Value = new PdfTextElement($"   Cliente: {selectedRecord.CLIENTE}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    ...
                    //Add RowSpan
                    var gridCell16 = pdfGrid.Rows[5].Cells[4];
                    gridCell16.ColumnSpan = 2;
                    gridCell16.StringFormat = Izquierda;
                    gridCell16.Value = new PdfTextElement($"   Remito: {selectedRecord.REMITO}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell14 = pdfGrid.Rows[6].Cells[0];
                    gridCell14.ColumnSpan = 3;
                    gridCell14.StringFormat = Izquierda;
                    gridCell14.Value = $"   Mantenimiento anterior: {selectedRecord.PEDIDOANT}";
                    //Add RowSpan
                    var gridCell15 = pdfGrid.Rows[6].Cells[3];
                    ...
                    //Add RowSpan
                    var gridCell17 = pdfGrid.Rows[7].Cells[0];
                    gridCell17.ColumnSpan = 6;
                    gridCell17.StringFormat = Centrado;
                    gridCell17.Value = new PdfTextElement("DATOS DE PLACA", font, new PdfPen(PdfColor.Empty),
                        PdfBrushes.Black, Centrado);
                    //Add RowSpan
                    var gridCell18 = pdfGrid.Rows[8].Cells[0];
                    gridCell18.ColumnSpan = 3;
                    gridCell18.StringFormat = Izquierda;
                    gridCell18.Value = $"   TAG: {selectedRecord.IDENTIFICACION}";
                    //Add RowSpan
                    var gridCell19 = pdfGrid.Rows[8].Cells[3];
                    gridCell19.ColumnSpan = 3;
                    gridCell19.StringFormat = Izquierda;
                    gridCell19.Value = new PdfTextElement($"   Marca: {selectedRecord.MARCA}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell20 = pdfGrid.Rows[9].Cells[0];
                    gridCell20.ColumnSpan = 3;
                    gridCell20.StringFormat = Izquierda;
                    gridCell20.Value = new PdfTextElement($"   Número de serie: {selectedRecord.NSERIE}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell21 = pdfGrid.Rows[9].Cells[3];
                    gridCell21.ColumnSpan = 3;
                    gridCell21.StringFormat = Izquierda;
                    gridCell21.Value = new PdfTextElement($"   Modelo: {selectedRecord.MODELO}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell22 = pdfGrid.Rows[10].Cells[0];
                    gridCell22.ColumnSpan = 3;
                    gridCell22.StringFormat = Izquierda;
                    gridCell22.Value = new PdfTextElement($"   Medida: {selectedRecord.MEDIDA}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell23 = pdfGrid.Rows[10].Cells[3];
                    gridCell23.ColumnSpan = 3;
                    gridCell23.StringFormat = Izquierda;
                    gridCell23.Value = new PdfTextElement($"   Clase: {selectedRecord.SERIE}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell24 = pdfGrid.Rows[11].Cells[0];
                    gridCell24.ColumnSpan = 3;
                    gridCell24.StringFormat = Izquierda;
                    gridCell24.Value = new PdfTextElement($"   Orificio: {selectedRecord.ORIFICIO}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell25 = pdfGrid.Rows[11].Cells[3];
                    gridCell25.ColumnSpan = 3;
                    gridCell25.StringFormat = Izquierda;
                    gridCell25.Value = new PdfTextElement($"   Año: {selectedRecord.AÑO}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell26 = pdfGrid.Rows[12].Cells[0];
                    gridCell26.ColumnSpan = 3;
                    gridCell26.StringFormat = Izquierda;
                    gridCell26.Value = new PdfTextElement($"   Area: {selectedRecord.AREA}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell27 = pdfGrid.Rows[12].Cells[3];
                    gridCell27.ColumnSpan = 3;
                    gridCell27.StringFormat = Izquierda;
                    gridCell27.Value = $"   Fluido: {selectedRecord.FLUIDO.Trim()}";
                    //Add RowSpan
                    var gridCell28 = pdfGrid.Rows[13].Cells[0];
                    gridCell28.ColumnSpan = 3;
                    gridCell28.StringFormat = Izquierda;
                    gridCell28.Value = $"   Sobrepresión: {selectedRecord.SOBREPRESION}%";
                    //Add RowSpan
                    var gridCell29 = pdfGrid.Rows[13].Cells[3];
                    gridCell29.ColumnSpan = 3;
                    gridCell29.StringFormat = Izquierda;
                    gridCell29.Value = new PdfTextElement($"   Presión: {selectedRecord.PRESION.Trim()} Bar", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell30 = pdfGrid.Rows[14].Cells[0];
                    ...
                    //Add RowSpan
                    var gridCell31 = pdfGrid.Rows[14].Cells[3];
                    gridCell31.ColumnSpan = 3;
                    gridCell31.StringFormat = Izquierda;
                    gridCell31.Value = $"   Tipo: {selectedRecord.TIPO}";
                    //Add RowSpan
                    var gridCell32 = pdfGrid.Rows[15].Cells[0];
                    gridCell32.ColumnSpan = 3;
                    gridCell32.StringFormat = Izquierda;
                    gridCell32.Value = $"   Temperatura: {selectedRecord.TEMP}";
                    //Add RowSpan
                    var gridCell33 = pdfGrid.Rows[15].Cells[3];
                    gridCell33.ColumnSpan = 3;
                    gridCell33.StringFormat = Izquierda;
                    gridCell33.Value = $"   Resorte: {selectedRecord.RESORTE}";
                    //Add RowSpan
                    var gridCell34 = pdfGrid.Rows[16].Cells[0];
                    gridCell34.ColumnSpan = 3;
                    gridCell34.StringFormat = Izquierda;
                    gridCell34.Value =
                        new PdfTextElement($"   Presión en Banco: {selectedRecord.PRESIONBANCO.Trim()} Bar", font,
                            new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell35 = pdfGrid.Rows[16].Cells[3];
                    gridCell35.ColumnSpan = 3;
                    gridCell35.StringFormat = Izquierda;
                    gridCell35.Value = $"   Servicio: {selectedRecord.SERVICIO}";
                    //Add RowSpan
                    var gridCell36 = pdfGrid.Rows[17].Cells[0];
                    gridCell36.ColumnSpan = 6;
                    gridCell36.StringFormat = Centrado;
                    gridCell36.Value = new PdfTextElement("ENSAYOS A LA RECEPCIÓN", font, new PdfPen(PdfColor.Empty),
                        PdfBrushes.Black, Centrado);
                    //Add RowSpan
                    var gridCell37 = pdfGrid.Rows[18].Cells[0];
                    gridCell37.ColumnSpan = 3;
                    gridCell37.StringFormat = Izquierda;
                    gridCell37.Value = new PdfTextElement($"   Ensayo a la Recepción: {selectedRecord.ENSRECEP}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell38 = pdfGrid.Rows[18].Cells[3];
                    gridCell38.ColumnSpan = 3;
                    gridCell38.StringFormat = Izquierda;
                    gridCell38.Value = new PdfTextElement($"   Estado: {selectedRecord.ESTADO}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell39 = pdfGrid.Rows[19].Cells[0];
                    gridCell39.ColumnSpan = 3;
                    gridCell39.StringFormat = Izquierda;
                    gridCell39.Value =
                        new PdfTextElement($"   Presión ensayo recepción: {selectedRecord.PRESIONRECEP.Trim()} Bar",
                            font, new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell40 = pdfGrid.Rows[19].Cells[3];
                    gridCell40.ColumnSpan = 3;
                    gridCell40.StringFormat = Izquierda;
                    gridCell40.Value = $"   Fugas: {selectedRecord.FUGAS}";
                    //Add RowSpan
                    var gridCell41 = pdfGrid.Rows[20].Cells[0];
                    ...
                    //Add RowSpan
                    var gridCell42 = pdfGrid.Rows[20].Cells[3];
                    gridCell42.ColumnSpan = 3;
                    gridCell42.StringFormat = Izquierda;
                    gridCell42.Value = new PdfTextElement($"   Cambio de presión: {selectedRecord.CAMBIOPRESION}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell43 = pdfGrid.Rows[21].Cells[0];
                    gridCell43.ColumnSpan = 6;
                    gridCell43.StringFormat = Centrado;
                    gridCell43.Value = new PdfTextElement("TRABAJOS EFECTUADOS", font, new PdfPen(PdfColor.Empty),
                        PdfBrushes.Black, Centrado);
                    //Add RowSpan
                    var gridCell44 = pdfGrid.Rows[22].Cells[0];
                    gridCell44.ColumnSpan = 3;
                    gridCell44.StringFormat = Izquierda;
                    gridCell44.Value = new PdfTextElement($"   Cambio de repuestos: {selectedRecord.CAMBIOREPUESTO}",
                        font, new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell45 = pdfGrid.Rows[22].Cells[3];
                    gridCell45.ColumnSpan = 3;
                    gridCell45.StringFormat = Izquierda;
                    gridCell45.Value = new PdfTextElement($"   Repuestos: {selectedRecord.REPUESTOS}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell46 = pdfGrid.Rows[23].Cells[0];
                    gridCell46.ColumnSpan = 3;
                    gridCell46.StringFormat = Izquierda;
                    gridCell46.Value = new PdfTextElement($"   Código de resorte: {selectedRecord.CODRESORTE}", font,
                        new PdfPen(PdfColor.Empty), PdfBrushes.Black, Izquierda);
                    //Add RowSpan
                    var gridCell47 = pdfGrid.Rows[23].Cells[3];
                    ...
                    //Add RowSpan
                    var gridCell48 = pdfGrid.Rows[24].Cells[0];
                    gridCell48.ColumnSpan = 6;
                    gridCell48.StringFormat = Izquierda;
                    if (selectedRecord.TRABAJOSEFEC == "D")
                        gridCell48.Value =
                            "   Trabajos efectuados: Desarme, limpieza, revisión de sus componentes, reacondicionamiento de asientos, rearmado, prueba, calibración.";
                    else if (selectedRecord.TRABAJOSEFEC == "V")
                        gridCell48.Value =
                            "   Trabajos efectuados: Unicamente verificación de funcionamiento y calibración.";
                    else
                        gridCell48.Value = $"   Trabajos efectuados: {selectedRecord.TRABAJOSEFEC}";
                    //Add RowSpan
                    var gridCell49 = pdfGrid.Rows[25].Cells[0];
        ...
        if (args.Item.Text == "OPDS")
            if (Grid.SelectedRecords.Count > 0)
                ...
                {
                    ClienteList =
                        await Http.GetFromJsonAsync<List<Cliente>>(
                            $"api/Cliente/BuscarPorCliente/{selectedRecord.CG_CLI}");
                    var document1 = new PdfDocument();
                    document1.PageSettings.Size = new SizeF(612, 1009);
                    ...
                    var espaciosEmail = "";
                    for (var i = 0; i < 38 - ClienteList.FirstOrDefault().DES_PROV.Trim().Length; i++)
                        espaciosEmail = espaciosEmail + " ";
                    var email = "";
                    if (ClienteList.FirstOrDefault().EMAIL.Trim().Length > 29)
                        email = ClienteList.FirstOrDefault().EMAIL;
                    var espaciosMedida = "";
                    for (var i = 0; i < 46 - selectedRecord.IDENTIFICACION.Trim().Length; i++)
                        espaciosMedida = espaciosMedida + " ";
                    var espaciosSerie = "";
                    for (var i = 0; i < 42 - selectedRecord.MARCA.Trim().Length; i++)
                        espaciosSerie = espaciosSerie + " ";
                    var espaciosFluido = "";
                    for (var i = 0; i < 57 - selectedRecord.MODELO.Trim().Length; i++)
                        espaciosFluido = espaciosFluido + " ";
                    var espaciosTemperatura = "";
                    for (var i = 0; i < 60 - selectedRecord.MODELO.Trim().Length; i++)
                    ...
                    if (selectedRecord.FUGAS.Trim() == "SI")
                        ...
                        fugas = "     x";
                    var espaciosfugas = "";
                    for (var i = 0; i < 24 - fugas.Length; i++) espaciosfugas = espaciosfugas + " ";
                    string TrEfec;
                    string TrEfec2;
                    if (selectedRecord.TRABAJOSEFEC == "D" ||
                        selectedRecord.TRABAJOSEFEC.Contains("reacondicionamiento"))
                    {
                        TrEfec = "Desarme, limpieza, revisión de sus componentes,";
                        TrEfec2 = "reacondicionamiento de asientos, rearmado, prueba, calibración.";
                    }
                    else if (selectedRecord.TRABAJOSEFEC == "V" ||
                             selectedRecord.TRABAJOSEFEC.Contains("funcionamiento"))
                    {
                    ...
                    await JS.SaveAs($"{selectedRecord.PEDIDO} OPDS {selectedRecord.ACTA.Trim()}" + ".pdf",
                        xx.ToArray());
                }
