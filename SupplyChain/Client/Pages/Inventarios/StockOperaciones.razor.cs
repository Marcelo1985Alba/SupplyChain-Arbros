...
{
    protected bool abrioVale;

    protected bool AbrirBuscadorResumenStockAutomaticamente;
    ...
    [Parameter] public bool EsEntrega { get; set; }
    [Parameter] public bool FiltraComboOperaciones { get; set; }
    [Parameter] public int OrdFab { get; set; }
    [Parameter] public PedidoEncabezado StockEncabezado { get; set; } = new();
    [Parameter] public int vale { get; set; }
    [Parameter] public bool AplicarFiltro { get; set; }
    [Parameter] public string cg_mat { get; set; } = string.Empty;
    [Parameter] public string despacho { get; set; } = string.Empty;

    protected bool DisableCssClass => StockEncabezado.TIPOO == 0 ? true : false;

    protected bool SpinnerVisible { get; set; }

    protected bool PermiteAgregarItem { get; set; }
    protected bool PermiteEditarItem { get; set; }
    protected bool PermiteEliminarItem { get; set; }
    ...
    {
        if (StockEncabezado?.VALE == 0) StockEncabezado.VALE = await InventarioService.GetProximoVale();
    }
    ...
    protected async Task NuevoVale()
    {
        ...
        abrioVale = false;
        StockEncabezado = new PedidoEncabezado();
    ...
    protected async Task SelectedTireChanged(Tire tire)
    {
        ...
        if (tire.Tipoo == 5) //recep prove
        {
            ...
            foreach (var item in StockEncabezado.Items) item.CG_DEP = 4;
        }
    ...
    protected void SelectedDepositoSalidaChanged(Deposito deposito)
    {
        ...
        if (StockEncabezado.TIPOO == 6) //DEVOL prove
            ...
            StockEncabezado.Items.ForEach(s => { s.CG_DEP = deposito.CG_DEP; });
    }
    ...
    protected async Task CargarVale()
    {
        ...
        {
            Cg_CLI_Cg_PROVE = StockEncabezado.Items[0].CG_CLI == 0
                ? StockEncabezado.Items[0].CG_PROVE
                : StockEncabezado.Items[0].CG_CLI;
            DescripcionPro = StockEncabezado.Items[0].CG_CLI == 0
                ? StockEncabezado.Items[0].Proveedor.DESCRIPCION.Trim()
                : "Cliente";
            ...
            await StockEncabezado.Items.ForEachAsync(async i =>
            {
                ...
                var prod = await Http.GetFromJsonAsync<Producto>(
                    $"api/Prod/GetByFilter?Codigo={filterProd.Codigo}&Descripcion={filterProd.Descripcion}");
        ...
        if (StockEncabezado.TIPOO == 10 || StockEncabezado.TIPOO == 28)
            await StockEncabezado.Items.ForEachAsync(async i => { i.STOCK = Math.Abs((decimal)i.STOCK); });
    ...
    protected async Task Guardar()
    {
        ...
        await StockEncabezado.Items.ForEachAsync(async s => { await SetearCamposParaGuardar(s); });
    ...
    protected async Task SetearCamposParaGuardar(Pedidos stock)
    {
        ...
        if (StockEncabezado.TIPOO == 21) stock.AVISO = "AJUSTE INVENTARIOS";
        ...
        if (StockEncabezado.TIPOO == 28) stock.AVISO = "ENTREGA A UNA ORDEN DE ARMADO";
    ...
    {
        if (StockEncabezado.TIPOO == 5)
            await StockEncabezado.Items.ForEachAsync(async s => { await PdfService.EtiquetaRecepcion(s); });
        else if (StockEncabezado.TIPOO == 9)
            await StockEncabezado.Items.ForEachAsync(async s =>
            {
                if (s.STOCK > 0) await PdfService.EtiquetaMovimiento(s);
            });
    }
    ...
    protected async Task onResumenStockSelected(ResumenStock item)
    {
        ...
        var lStock = new List<Pedidos>();
        var registronegativo = 0;
    ...
    protected async Task OnProgramaSelected(Programa programaSel)
    {
        ...
        StockEncabezado.ModeloOrdenFabricacionEncabezado =
    ...
    protected async Task OnCompraSelected(List<Compra> lcompraSel)
    {
        ...
        if (StockEncabezado.TIPOO == 6)
        {
            ...
            StockEncabezado.Items =
                await Http.GetFromJsonAsync<List<Pedidos>>($"api/Stock/AbriValeByOCParaDevol/{lcompraSel[0].NUMERO}");
    ...
    protected async Task OnResumenStockSelected(Pedidos stockSel)
    {
        ...
        if (StockEncabezado.Items != null || StockEncabezado.Items.Count > 0) await CargarVale();
    ...
    private bool popupBuscadorVisible;

    protected bool PopupBuscadorVisible
    {
        get => popupBuscadorVisible;
        set
        {
            popupBuscadorVisible = value;
            InvokeAsync(StateHasChanged);
        }
    }

    private bool popupBuscadorVisibleRS;

    protected bool PopupBuscadorVisibleRS
    {
        get => popupBuscadorVisibleRS;
        set
        {
            popupBuscadorVisibleRS = value;
            InvokeAsync(StateHasChanged);
        }
    }

    protected string[] ColumnasBuscador; /*{ "VALE", "FE_MOV", "CG_ART", "DES_ART", "DESPACHO", "LOTE", "SERIE" };*/
    ...
    protected async Task BuscarVales()
    {
        ...
        ColumnasBuscador = new[] { "VALE", "FE_MOV", "CG_ART", "DES_ART", "STOCK" };
