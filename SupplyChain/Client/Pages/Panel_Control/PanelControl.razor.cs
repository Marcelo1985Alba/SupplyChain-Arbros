...
public class BasePanelControl : ComponentBase
{
    ...
    protected int añoSeleccionadoPedidosAltasDiasAtraso;
    ...
    ///////***********************MAYOR*************////////////////////////////
    protected List<vMayorItris> DataSourceMayor = new();

    protected List<vMayorItris> DataSourceMayorFiltrado = new();

    ///////**********************************************////////////////////////////
    /////////////**********MERCADO EXTERNO*********////////////////////
    protected DialogDetalle dialogDetalle;
    ...
    protected int PromedioComprasMensual;
    protected int PromedioFacturacionMensual;
    protected int PromedioPedidosAltaDiasAtrasoMensual;
    protected int PromedioPedidosIngresadosMensuales;
    protected int PromedioPedidosIngresadosMensualesUE;
    protected int PromedioPresupuestosMensuales;

    public Query Qry = new Query().AddParams("Id", 10).AddParams("TaskName", "Test2")
        .AddParams("StartDate", DateTime.Now)
    ...
    protected double[] Spacing = { 15, 15 };
    ...
    {
        DataFacturacionOriginal =
            await Http.GetFromJsonAsync<List<vEstadFacturacion>>("api/EstadisticaVentas/Facturacion");
    ...
    {
        DataComprasOriginal = await Http.GetFromJsonAsync<List<vEstadCompras>>("api/EstadisticaVentas/Compras");
    ...
    protected async Task GetPedidosAltas()
    {
        ...
        PedidosAltasAnualDiasAtraso = DataPedidosAltas.Where(p => p.PEDIDO > 0)
            .Select(d => new ChartData
            {
                ...
                YSerieName = Math.Abs((d.FECHA_PREVISTA.HasValue
                    ? d.FECHA_PREVISTA.Value - d.FECHA_DATE
                    : DateTime.Now - d.FECHA_DATE).TotalDays)
            })
    ...
    {
        DataPresupuestos = await Http.GetFromJsonAsync<List<vEstadPresupuestos>>("api/EstadisticaVentas/Presupuestos");
    ...
    public async Task ValueChangeFechasFacturacion(RangePickerEventArgs<DateTime> args)
    {
        ...
        FacturacionAnual = DataFacturacionOriginal
        ...
        //para grilla de detalle
        grdFacturacionDetalle.PreventRender();
        DataFacturacionDetalle = new List<vEstadFacturacion>();
        DataFacturacionDetalle = DataFacturacionOriginal
            .Where(f => f.FECHA >= FacturacionMinDate && f.FECHA <= FacturacionMaxDate)
            .OrderBy(o => o.FECHA)
            .ToList();
    ...
    {
        VisibleSpinner = true;
        FacturacionAnual = new List<ChartData>();
        FacturacionMensual = new List<ChartData>();
        DataFacturacionDetalle = new List<vEstadFacturacion>();
    ...
    {
        VisibleSpinner = true;
        ComprasAnual = new List<ChartData>();
        ComprasMensual = new List<ChartData>();
        DataComprasDetalle = new List<vEstadCompras>();
        ComprasMensualTipo = new List<ChartData>();
    ...
    {
        VisibleSpinner = true;
        EventosAnual = new List<ChartData>();
        EventosMensual = new List<ChartData>();
        DataEventosDetalle = new List<vEstadEventos>();
        EventosMensualTipo = new List<ChartData>();
        EventosProveedor = new List<ChartData>();
        await GetEventos();
        VisibleSpinner = false;
    }
    ...
    {
        VisibleSpinner = true;
        PedidosIngresadosAnuales = new List<ChartData>();
        PedidosIngresadosMensuales = new List<ChartData>();
        PedidosIngresadosMercado = new List<ChartData>();
        PedidosIngresadosMensualesCategoriaDetalle = new List<ChartData>();
        PedidosIngresadosDetalleMes = new List<vEstadPedidosIngresados>();
        PedidosIngresadosCategoria = new List<ChartData>();
        PromedioPedidosIngresadosMensuales = 0;
        PedidosIngresadosGarantia = new List<ChartData>();
    ...
    {
        VisibleSpinner = true;
        DataPedidosAltasDiasAtrasoDetalle = new List<vEstadPedidosAlta>();
        PromedioPedidosAltaDiasAtrasoMensual = 0;
        PedidosAltasAnualDiasAtraso = new List<ChartData>();
        PedidosAltasMensualDiasAtraso = new List<ChartData>();
        await GetPedidosAltas();
        VisibleSpinner = false;
    }
    ...
    {
        VisibleSpinner = true;
        DataPedidosIngresadosUEDetalle = new List<vEstadPedidosIngresados>();
        PedidosIngresadosUEAnuales = new List<ChartData>();
        PedidosIngresadosUEMensuales = new List<ChartData>();
    ...
    {
        VisibleSpinner = true;
        PresupuestosAnuales = new List<ChartData>();
        PresupuestosMensuales = new List<ChartData>();
        PresupuestosAnualesDetalle = new List<vEstadPresupuestos>();
    ...
    public async Task ValueChangeFechasPedidosIngresados(RangePickerEventArgs<DateTime> args)
    {
        ...
        TituloGraficoPedidosMensual =
            $"U$S Mensuales Desde {PedidosIngresadosMinDate:dd/MM/yyyy} Hasta {PedidosIngresadosMaxDate:dd/MM/yyyy}";
        TituloPedidosIngresadosMercado =
            $"Mercado Externo Desde {PedidosIngresadosMinDate:dd/MM/yyyy} Hasta {PedidosIngresadosMaxDate:dd/MM/yyyy}";
        //grafico mensual
        ...
        TituloGraficoGarantia =
            $"Pedidos Garantia Periodo {PedidosIngresadosMinDate:dd/MM/yyyy} {PedidosIngresadosMaxDate:dd/MM/yyyy}";

        PedidosIngresadosGarantia = DataPedidosIngresados.Where(p => p.FECHA >= PedidosIngresadosMinDate
                                                                     && p.FECHA <= PedidosIngresadosMaxDate &&
                                                                     p.DES_ART.ToLower().Contains("garan"))
        ...
        TituloPedidosIngresadosCategoria =
            $"% por Categoria Desde {PedidosIngresadosMinDate:dd/MM/yyyy} Hasta {PedidosIngresadosMaxDate:dd/MM/yyyy}\n" +
        ...
        PedidosIngresadosCategoria = DataPedidosIngresados
            ...
            .Select(d => new ChartData
            {
                ...
                ZSerieName =
                    $"{Math.Round(Math.Round(d.Sum(p => p.TOT_DOL), 2) / totalPeriodoSeleccionado, 2) * 100} %\n" +
    ...
    {
        TituloGraficoPedidosIngresadosUEMensual =
            $"Desde {PedidosIngresadosMinDate:dd/MM/yyyy} Hasta {PedidosIngresadosMaxDate:dd/MM/yyyy}";
        //grafico mensual
    ...
    {
        TituloGraficoPresupuestosMensual =
            $"U$S Mensuales {PresupuestosMinDate:dd/MM/yyyy} {PresupuestosMaxDate:dd/MM/yyyy}";
        ...
        PresupuestosAnualesDetalle = DataPresupuestos
            .Where(p => p.FECHA >= PresupuestosMinDate && p.FECHA <= PresupuestosMaxDate)
            .OrderBy(p => p.FECHA)
            .ToList();
    ...
    public async Task ValueChangeFechasDiasAtraso(RangePickerEventArgs<DateTime> args)
    {
        ...
        PedidosAltasAnualDiasAtraso = DataPedidosAltas.Where(p =>
                p.PEDIDO > 0 && p.FECHA >= DiasAtrasoMinDate && p.FECHA <= DiasAtrasoMaxDate)
            .Select(d => new ChartData
            {
                XSerieName = d.ANIO.ToString(),
                YSerieName = Math.Abs((d.FECHA_PREVISTA.HasValue
    ...
    protected async Task MostrarDetalleCompras(PointEventArgs args)
    {
        var año = args.Point.X;
    ...
    protected async Task MostrarDetalleComprasMesTipo(PointEventArgs args)
    {
    ...
    protected async Task MostrarDetallePedidosAnuales(PointEventArgs args)
    {
        ...
        PedidosIngresadosGarantia = DataPedidosIngresados.Where(p =>
                p.ANIO == añoSeleccionadoPedidosIngresados && p.DES_ART.ToLower().Contains("garan"))
            .GroupBy(g => new { g.DES_ART })
            .Select(d => new ChartData
            {
                XSerieName = d.Key.DES_ART,
        ...
        TituloPedidosIngresadosCategoria =
            $"% por Categoria del {añoSeleccionadoPedidosIngresados} (US$ {totalPeriodoSeleccionado})";
    ...
    protected async Task MostrarDetallePedidosAnualesVs(PointEventArgs args)
    {
    }

    protected async Task MostrarDetallePedidosMensuales(PointEventArgs args)
    {
        ...
        var nombreMes =
            new DateTime(añoSeleccionadoPedidosIngresados, mesSeleccionadoPedidosIngresados, 1).ToString("MMMM");
        TituloPedidosIngresadosCategoria =
            $"% por Categoria de {nombreMes} del {añoSeleccionadoPedidosIngresados} (US$ {totalPeriodoSeleccionado})";
        ...
        PedidosIngresadosGarantia = DataPedidosIngresados.Where(p => p.ANIO == añoSeleccionadoPedidosIngresados
                                                                     && p.MES == mesSeleccionadoPedidosIngresados &&
                                                                     p.DES_ART.ToLower().Contains("garan"))
            .GroupBy(g => new { g.DES_ART })
            .Select(d => new ChartData
            {
    ...
    protected async Task MostrarDetallePresupuestosAnuales(PointEventArgs args)
    {
        var año = args.Point.X;
    ...
    protected async Task MostrarDetalleDiasAtraso(PointEventArgs args)
    {
        ...
        PedidosAltasMensualDiasAtraso = DataPedidosAltas
            .Where(p => p.PEDIDO > 0 && p.ANIO == añoSeleccionadoPedidosAltasDiasAtraso)
            .OrderBy(p => p.FECHA)
            .Select(d => new ChartData
            {
                XSerieName = d.MES.ToString(),
                YSerieName = (d.FECHA_PREVISTA.HasValue
                    ? d.FECHA_PREVISTA.Value - d.FECHA_DATE
                    : DateTime.Now - d.FECHA_DATE).TotalDays
            })
            .GroupBy(g => new { g.XSerieName })
        ...
        PromedioPedidosAltaDiasAtrasoMensual =
            Convert.ToInt32(PedidosAltasMensualDiasAtraso.Average(d => d.YSerieName));

        DataPedidosAltasDiasAtrasoDetalle = DataPedidosAltas
            .Where(p => p.PEDIDO > 0 && p.ANIO == añoSeleccionadoPedidosAltasDiasAtraso).OrderBy(p => p.FECHA).ToList();
        await InvokeAsync(StateHasChanged);
    ...
    protected async Task MostrarDetalleDiasAtrasoMes(PointEventArgs args)
    {
        ...
        DataPedidosAltasDiasAtrasoDetalle = DataPedidosAltas.Where(p => p.PEDIDO > 0
                                                                        && p.ANIO ==
                                                                        añoSeleccionadoPedidosAltasDiasAtraso &&
                                                                        p.MES == mes).OrderBy(p => p.FECHA).ToList();
        await InvokeAsync(StateHasChanged);

        await grdDiasAtraso.AutoFitColumnsAsync();
    }

    protected async Task MostrarDetalleUE(PointEventArgs args)
    {
    ...
    protected async Task MostrarDetalleUEMes(PointEventArgs args)
    {
        var mes = Convert.ToInt32(args.Point.X);
    ...
    protected async Task OnFiltroByCategoria(AccumulationPointEventArgs args)
    {
        ...
        TituloPedidosIngresadosCategoriaDetalle =
            $"Categoria {categoria.ToUpper()} de {añoSeleccionadoPedidosIngresados}";
    ...
    {
        DataProyectos = await Http.GetFromJsonAsync<List<GanttDataDetails>>("api/proyectos");
        //Gantt.PreventRender();
    }

    public void AddRow()
    {
        tarea = new GanttDataDetails();
        tarea.ESNUEVO = true;
        ShowDialog = true;
    ...
    {
        await Http.PostAsJsonAsync("/api/Proyectos", tarea);
        return true;
    }

    protected async Task<bool> Actualizar(GanttDataDetails tarea)
    {
        await Http.PutAsJsonAsync("/api/Proyectos/" + tarea.Id, tarea);
        return true;
    }
