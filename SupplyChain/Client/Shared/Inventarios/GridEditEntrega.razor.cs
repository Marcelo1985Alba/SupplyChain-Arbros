...
public class GridEditEntregaBase : ComponentBase
{
    ...
    private bool deshabilitaBotonBuscar;

    protected DialogSettings DialogParams = new() { MinHeight = "400px", Width = "900px" };
    ...
    protected Programa[] ordenesFabricacion;
    protected Programa[] ordenesPlaneadas;
    ...
    protected List<ItemModel> Toolbaritems = new()
    {
        new()
        {
            CssClass = "", Text = "Agregar Insumo", ShowTextOn = DisplayMode.Both, Type = ItemType.Button,
            TooltipText = "Agregar Insumo", SuffixIcon = "fa fa-search", Id = "AgregarInsumo"
        }
    };

    protected bool visibleSpinnerRS;
    [Inject] public IJSRuntime JsRuntime { get; set; }
    [Inject] private HttpClient Http { get; set; }
    [Parameter] public string Titulo { get; set; } = null!;
    [Parameter] public List<Pedidos> DataSource { get; set; } = null!;
    [Parameter] public bool PermiteAgregar { get; set; }
    [Parameter] public bool PermiteEditar { get; set; }
    [Parameter] public bool PermiteEliminar { get; set; }
    ...
    [Parameter] public bool AbrirBuscadorResumenStock { get; set; }
    ...
    {
        get => deshabilitaBotonBuscar;
        set { deshabilitaBotonBuscar = DataSource.Any(s => s.TIPOO == 9); }
    }

    protected string tituloBuscador { get; set; } = "";

    protected bool PopupBuscadorStockVisible
    {
    ...
    {
        if (AbrirBuscadorResumenStock) await AgregarInsumo();
    }

    public async Task ClickHandler(ClickEventArgs args)
    {
        if (args.Item.Id == "AgregarInsumo") await AgregarInsumo();
    }
    ...
    protected async Task AgregarInsumo()
    {
        ...
        ColumnasBuscador = new[] { "CG_ART", "DEPOSITO", "DESPACHO", "SERIE", "LOTE", "STOCK" };
    ...
    private void OnRowRemoving(Pedidos pedido)
    {
    ...
    protected async Task BuscarProductos()
    {
        ...
        ColumnasBuscador = new[] { "CG_PROD", "DES_PROD", "UNID" };
    ...
    protected async Task BuscarProductosStock()
    {
        ...
        PopupBuscadorStockVisible = true;
        ColumnasBuscador = new[] { "CG_ART", "PRODUCTO", "DEPOSITO", "DESPACHO", "SERIE", "LOTE", "STOCK" };
    ...
    protected async Task BuscarProductosStockEnPositivo()
    {
        ...
        ColumnasBuscador = new[] { "CG_ART", "PRODUCTO", "CG_DEP", "DEPOSITO", "DESPACHO", "SERIE", "LOTE", "STOCK" };
        //int cg_orden = TipoInsumoCodigo;
    ...
    protected void CantChange()
    {
        ...
        {
            if (stock.STOCK < 0) stock.PENDIENTEOC = stock.ResumenStock.STOCK - stock.STOCK * -1;
            if (stock.STOCK > 1) stock.PENDIENTEOC = stock.ResumenStock.STOCK + stock.STOCK;
        }
    }
    ...
    public async Task QueryCellInfoHandler(QueryCellInfoEventArgs<Pedidos> args)
    {
        ...
        if (args.Data.TIPOO == 10 || args.Data.TIPOO == 28)
        {
            ...
            if (args.Data.ResumenStock?.STOCK < args.Data.STOCK) args.Cell.AddClass(new[] { "sin-stock" });
        }

        if (args.Data.STOCK == 0 && args.Data.StockPlaneado + args.Data.StockEnFabricacion - args.Data.CANTENT >= 0)
            args.Cell.AddClass(new[] { "sin-stock" });
    }
}