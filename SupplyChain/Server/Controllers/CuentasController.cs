...
public class CuentasController : ControllerBase
{
    ...
    public CuentasController(AppDbContext context, UserManager<ApplicationUser> userManager,
        ...
        IHubContext<OnlineUsersHub> hubContext)
    {
        ...
        _hubOnlineUsersContext = hubContext;
        _context = context;
    }
    ...
    public async Task<ActionResult> ResetearPassword(UserInfo userInfo)
    {
        ...
        if (user != null)
        {
            ...
            if (!result.Succeeded) return BadRequest("La contraseña no pudo ser reseteada!");
        }
    ...
    public async Task<ActionResult> AsignarRol(RolUser rolUser)
    {
        ...
        if (!result.Succeeded) return BadRequest("El Rol no pudo ser asigndo!");
    ...
    public async Task<ActionResult<UserToken>> CrearUsuario([FromBody] UserInfo userInfo)
    {
        var user = new ApplicationUser
            { UserName = userInfo.UserName, Cg_Cli = userInfo.Cg_Cli, Email = userInfo.Email };
        ...
        if (result.Succeeded)
        {
            if (userInfo.Roles.Count > 0)
                foreach (var rol in userInfo.Roles)
                {
                    ...
                    if (!resultRoles.Succeeded) return BadRequest(resultRoles.Errors.Select(s => s.Description));
                }
    ...
    public async Task<ActionResult<UserToken>> Login([FromBody] UserInfo userInfo)
    {
        ...
        if (result.Succeeded)
        {
            ...
            if (roles.Any(r => r == "Cliente") && user.Cg_Cli == 0) return BadRequest("Solicite autorizacion");
    ...
    {
        var claims = AgregaClaims(userInfo, roles);
        ...
        JwtSecurityToken token = new(null,
            null, claims, expires: expiration, signingCredentials: creds);
