...
public class TipoAreaController : ControllerBase
{
    ...
    {
        _tipoAreaRepository = tipoAreaRepository;
    }
    ...
    {
        if (id != tipoArea.Id) return BadRequest();
    ...
    public async Task<ActionResult<TipoArea>> DeleteTipoAreas(int id)
    {
        ...
        if (tipoArea == null) return NotFound();
    ...
    public async Task<ActionResult<TipoArea>> PostList(List<TipoArea> tipoAreas)
    {
        try
        {
            foreach (var item in tipoAreas) await _tipoAreaRepository.Remover(item.Id);
        }
        catch (Exception ex)
        {
            return BadRequest();
