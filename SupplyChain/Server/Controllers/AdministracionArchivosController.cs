...
public class AdministracionArchivosController : ControllerBase
{
    ...
    public async Task<ActionResult<List<Archivo>>> GetDocumentosByRuta(string parametro, string codigo)
    {
        try
        {
            ...
            {
                var pedido = codigo.Split(',')[0];

                const string FirstCharacter = "_";
                var Pos1 = pedido.IndexOf(FirstCharacter) + FirstCharacter.Length;
                pedido = pedido.Substring(Pos1);

                var Pos2 = pedido.IndexOf('_') + FirstCharacter.Length;
    ...
    private async Task<List<Archivo>> GetFileLocalServer(string parametro, string codigo)
    {
        ...
        var endLength = ruta.CAMPO.Trim() == "RUTAENSAYO" ? 9 :
            ruta.CAMPO.Trim() == "RUTACERTIFICADOS" || ruta.CAMPO.Trim() == "RUTATRAZABILIDAD"
                                                    || ruta.CAMPO.Trim() == "RUTAREMITO" ||
                                                    ruta.CAMPO.Trim() == "RUTAFACTURA" ? codigo.Length :
        ...
        try
        {
            ...
            if (ruta.CAMPO.Trim() == "RUTAENSAYO")
                ...
                file += ".pdf";
            else if (ruta.CAMPO.Trim() == "RUTACNC") file += "*";

            var dirs = Directory.GetFiles($"{ruta.VALORC}", $"{file}",
                new EnumerationOptions
                {
                    MatchCasing = MatchCasing.CaseInsensitive, MatchType = MatchType.Simple, IgnoreInaccessible = true
                });
            ...
            foreach (var item in dirs)
            {
                ...
                var archivo = new Archivo
                {
                    ...
                    ContenidoByte = parametro == "RUTACNC" || ruta.CAMPO.Trim() == "RUTACERTIFICADOS" ||
                                    ruta.CAMPO.Trim() == "RUTATRAZABILIDAD"
                                    || ruta.CAMPO.Trim() == "RUTAFACTURA" || ruta.CAMPO.Trim() == "RUTAREMITO"
    ...
    public async Task<ActionResult<ModeloOrdenFabricacion>> DownloadText(ModeloOrdenFabricacion ordfab)
    {
        ...
        var fileName = ruta.VALORC + ordfab.PEDIDO + ".txt";
        try
        {
    ...
    public async Task<IActionResult> GetPlano(string file)
    {
        try
        {
            ...
            var bytes = System.IO.File.ReadAllBytes(ruta.VALORC + "/" + file);
    ...
    public async Task<IActionResult> GetTxt(string filename)
    {
        try
        {
            var stream = new MemoryStream();
            var ruta = @"C:\Temp";
            var bytes = System.IO.File.ReadAllBytes(ruta + "/" + filename);
            stream = new MemoryStream(bytes);
    ...
    private static async Task<BlobContainerClient> GetContainer(string containerName)
    {
        ...
        var connectionString =
            "DefaultEndpointsProtocol=https;AccountName=arbrosstorage;AccountKey=d757aVPtZ0UFLcPfYn21vjEm4+FycBmm8q6OkvADT4lxWgBASGp/NeTR7eKYRBki4LwXF4RgfTRspa20VMxm/A==;EndpointSuffix=core.windows.net";
    ...
    public async Task<ActionResult<string>> GetPdfNube(string fileName, string ruta)
    {
        //Connection String of Storage Account
        var connectionString =
            "DefaultEndpointsProtocol=https;AccountName=blazorpdfplano;AccountKey=rAVlVam+dx56pfQknsxOEC72f5HqXJkN9bLa3ExOHpNUOpj5kO7ohcKGx4IMWMfO/egA70yy/q15uCnVv471fA==;EndpointSuffix=core.windows.net";
        //Container Name
        var containerName = "pdf-planos";
        ...
        foreach (var blob in cliente.GetBlobs())
        {
    ...
    {
        if (archivo == null) return BadRequest("El archivo es nulo");

        if (archivo.ContenidoByte == null) return BadRequest("Debe asignar stream");

        var path = await _solutionRepository.Obtener(s => s.CAMPO == "RUTADESPIECE").Select(s => s.VALORC)
            .FirstOrDefaultAsync();


        using (var memoryStream = new MemoryStream(archivo.ContenidoByte))
        {
            var tempFilePath = Path.Combine(path + archivo.Nombre);
    ...
    public async Task<IActionResult> Load([FromBody] Dictionary<string, string> jsonObject)
    {
        ...
        if (jsonObject != null && jsonObject.ContainsKey("document"))
        {
            ...
            {
                var bytes = Convert.FromBase64String(jsonObject["document"]);
                stream = new MemoryStream(bytes);
            }
        }
    ...
    {
        var pdfviewer = new PdfRenderer(_cache);
        var jsonResult = pdfviewer.ExportFormFields(jsonObject);
        return Content(jsonResult);
    }
    ...
    {
        var pdfviewer = new PdfRenderer(_cache);
        var jsonResult = pdfviewer.ImportFormFields(jsonObject);
        return Content(JsonConvert.SerializeObject(jsonResult));
    }
    ...
    {
        PdfRenderer pdfviewer = new(_cache);
        var documentBase = pdfviewer.GetDocumentAsBase64(jsonObject);
    ...
    public IActionResult PrintImages([FromBody] Dictionary<string, string> jsonObject)
    {
        //Initialize the PDF Viewer object with memory cache object
        var pdfviewer = new PdfRenderer(_cache);
        var pageImage = pdfviewer.GetPrintImage(jsonObject);
    ...
    public async Task<FileResult> Descargar(Archivo archivo)
    {
        ...
        //return archivos;
        return File(archivo.ContenidoByte, MediaTypeNames.Application.Pdf, archivo.Nombre);
    }
    ...
    public async Task<ActionResult> MergePdf(int nroPedido)
    {
        try
        {
            ...
            if (lineasCertificados.Count == 0) return NotFound();


            listArchivosDescargar.Add(lineasCertificados[0]);
            if (listaTrazabilidad.Count > 0)
            {
                var lineasRoscada = new List<int>(new[] { 8, 18, 52, 23 });
                var lineasBridada = new List<int>(new[] { 8, 18, 23, 19 });
            ...
            var streams = new Stream[listArchivosDescargar.Count];
            var finalDoc = new PdfDocument();
            for (var i = 0; i < listArchivosDescargar.Count; i++)
            ...
            stream.Position = 0;
            finalDoc.Close(true);
            for (var i = 0; i < listArchivosDescargar.Count; i++) streams[i].Dispose();
            var contentType = "application/pdf";
            var fileName = $"{nroPedido}.pdf";
    ...
    public async Task<ActionResult> SolicitudDatSheetPdf(int solicitudId)
    {
        ...
        localReportDS.AddDataSource("DataSet1", dataSheets); //debere recibir una lista
        var resultDS = localReportDS.Execute(RenderType.Pdf);
        ...
        var contentType = "application/pdf";
        return new FileContentResult(streamCot.ToArray(), contentType)
            { FileDownloadName = $"DataSheet_{solicitudId}.pdf" };
    ...
    public async Task<ActionResult> PresupuestoDataSheetPdf(int presupuesto)
    {
        ...
        var path = EliminarArchivos();

        //Obtener pdf presupuesto
        var presup = await _presupuestoRepository.Obtener(v => v.Id == presupuesto).Include(i => i.Items)
            .FirstOrDefaultAsync();
        var listArchivosDescargar = new List<Archivo>();
        ...
        var resultCC = localReportCC.Execute(RenderType.Pdf);
        ...
        for (var i = 0; i < listArchivosDescargar.Count; i++) streams[i].Dispose();
        var contentType = "application/pdf";
        var fileName = $"AR-CO-{presupuesto}.pdf";
