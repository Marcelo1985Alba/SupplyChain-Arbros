...
[ApiController]
public class CargasViejoController : ControllerBase
{
    ...
    public CargasViejoController(AppDbContext context)
    {
        _context = context;
    }
    ...
    // GET: api/Cargas
    [HttpGet]
    public IEnumerable<ModeloCargaViejo> Get()
    {
        try
        {
            // Busca turno
            ...
            var xTabla =
                xConexionSQL.EjecutarSQL("Select Top 1 ValorN From Solution Where Campo = 'HORASDIA'");
            if (xTabla.Rows.Count > 0) CantidadColumnasPorPeriodo = Convert.ToInt16(xTabla.Rows[0]["ValorN"]);
            ...
            if (xTabla.Rows.Count > 0) xExtensionColumnas = Convert.ToInt16(xTabla.Rows[0]["ValorN"]);

            var xAvanceAutomatico = 0;
            xConexionSQL = new ConexionSQL(CadenaConexionSQL);
            xTabla = xConexionSQL.EjecutarSQL(
                "SELECT ValorN FROM solution WHERE Campo = 'CARGA_AVANCE_AUTOMATICO'");
            if (xTabla.Rows.Count > 0) xAvanceAutomatico = Convert.ToInt32(xTabla.Rows[0]["ValorN"]);
            ...
            var xSQLSelect =
                "SELECT B.CG_ORDEN, A.CG_ESTADOCARGA, A.CG_ORDF, CG_ORDFORIG, CG_ORDFASOC, LTRIM(RTRIM(A.CG_CELDA)) AS CG_CELDA, C.DES_CELDA, " +
            ...
            xConexionSQL = new ConexionSQL(CadenaConexionSQL);
            var dbCargaTemp = xConexionSQL.EjecutarSQL(xSQLSelect);
            DateTime xFecha;
            if (dbCargaTemp.Rows.Count == 0) return new List<ModeloCargaViejo>();
            ...
            xConexionSQL = new ConexionSQL(CadenaConexionSQL);
            var dbFestivos = xConexionSQL.EjecutarSQL("SELECT Fecha FROM CalendarioFestivos ORDER BY Fecha");
            ...
            foreach (DataRow xRowCarga in dbCarga.Rows)
            {
                foreach (DataRow xRowCargaColores in dbCarga.Rows)
                    ...
                    if (xRowCarga["CG_ORDFASOC"].ToString() == xRowCargaColores["CG_ORDFASOC"].ToString() &&
                        xRowCargaColores["BACKGROUND"].ToString() != "")
                    {
            ...
            while (i < dbCarga.Rows.Count)
            {
                ...
                while (i < dbCarga.Rows.Count && xCg_celda == dbCarga.Rows[i]["CG_CELDA"].ToString().Trim())
                {
                    ...
                    if (xAvanceAutomatico == 1)
                    {
                        ...
                        {
                            xDiasPasados = (DateTime.Now -
                                            Convert.ToDateTime(
                                                dbCarga.Rows[i]["FECHA_PREVISTA_FABRICACION"])).Days;
                            if (xDiasPasados > 0)
                            {
                            ...
                            {
                                xHorasPasadas = DateTime.Now.Hour -
                                                Convert.ToDateTime(dbCarga.Rows[i]["FECHA_PREVISTA_FABRICACION"])
                                                    .Hour;
                    ...
                    else
                    {
                        if (Convert.ToDecimal(dbCarga.Rows[i]["AVANCE"]) > 0)
                            xDiasFab = Convert.ToDecimal(dbCarga.Rows[i]["DIASFAB"]) -
                                       Convert.ToDecimal(dbCarga.Rows[i]["DIASFAB"]) *
                                       Convert.ToDecimal(dbCarga.Rows[i]["AVANCE"]) / 100;
                        else
                            xDiasFab = Convert.ToDecimal(dbCarga.Rows[i]["DIASFAB"]);
                    }
                    ...
                    {
                        xColumnSpan = Convert.ToInt32(xDiasFab * CantidadColumnasPorPeriodo);
                        if (xHorasPasadas > 0) xColumnSpan -= xHorasPasadas;
                    }

                    if (xColumnSpan <= 0) xColumnSpan = 1;
            ...
            var xHayCambios1 = true;
            var xHayCambios2 = true;
            while (xHayCambios1 && xHayCambios2)
            {
            ...
            // Retorno
            var xLista = dbCarga.AsEnumerable().Select(m => new ModeloCargaViejo
            {
        ...
        {
            return new List<ModeloCargaViejo>();
        }
    }
            ...
            while (i < dbCarga.Rows.Count)
            {
                if (Convert.ToInt32(dbCarga.Rows[i]["FLAG_DEPENDENCIAS"]) == 0)
                    if (Convert.ToInt32(dbCarga.Rows[i]["CG_ORDFORIG"]) > 0)
                    {
                        ...
                        xDataRows = dbCarga.Select("CG_ORDFASOC = " + dbCarga.Rows[i]["CG_ORDFASOC"]);
                            ...
                            while (z < xTablaDependencias.Rows.Count)
                            {
                                if (Convert.ToInt32(xTablaDependencias.Rows[z]["CG_ORDFORIG"]) > 0)
                                    if (Convert.ToInt32(dbCarga.Rows[i]["COLUMNA"]) < xColumnaInicial &&
                                        dbCarga.Rows[i]["CG_ORDF"].ToString() ==
                                        xTablaDependencias.Rows[z]["CG_ORDF"].ToString())
                                    {
                                ...
                                xColumnaInicial = Convert.ToInt32(xTablaDependencias.Rows[z]["COLUMNA"]) +
                                                  Convert.ToInt32(xTablaDependencias.Rows[z]["COLUMNSPAN"]);
                                z++;
                            }
                        }
                    }
        ...
        try
        {
            ...
            while (i < dbCarga.Rows.Count)
            {
                ...
                while (i < dbCarga.Rows.Count && xCg_celda == dbCarga.Rows[i]["CG_CELDA"].ToString().Trim())
                {
                    ...
                    xColumnaInicial = Convert.ToInt32(dbCarga.Rows[i]["COLUMNA"]) +
                                      Convert.ToInt32(dbCarga.Rows[i]["COLUMNSPAN"]);
                    i++;
                }
            }
