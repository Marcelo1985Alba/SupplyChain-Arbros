...
public class PresupuestosController : ControllerBase
{
    ...
    {
        foreach (var item in presupuesto.Items) item.Id = 0;
    ...
    public async Task<ActionResult<Presupuesto>> DeleteCompra(int id)
    {
        try
        {
            var presup = await _presupuestoRepository.Obtener(p => p.Id == id).Include(p => p.Items)
                .FirstOrDefaultAsync();
            if (presup == null) return NotFound();

            if (await _presupuestoRepository.TienePedido(id)) return Conflict("El presupuesto tiene pedido asociado.");
