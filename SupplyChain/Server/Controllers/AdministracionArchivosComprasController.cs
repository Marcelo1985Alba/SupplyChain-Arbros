...
public class AdministracionArchivosComprasController : ControllerBase
{
    ...
    public async Task<ActionResult<List<Archivo>>> GetDocumentosByRutaCompras(string parametro, string codigo)
    {
        try
        {
            ...
            {
                var compras = codigo.Split(',')[0];

                const string FirstCharacter = "_";
                var Pos1 = compras.IndexOf(FirstCharacter) + FirstCharacter.Length;
                compras = compras.Substring(Pos1);

                var Pos2 = compras.IndexOf('_') + FirstCharacter.Length;
    ...
    public async Task<ActionResult> PresupuestoDataSheetPdf(int presupuesto)
    {
        ...
        foreach (var item in presup?.Items)
            if (item.SOLICITUDID > 0)
            {
                ...
                if (sol != null && sol.CalcId > 0) solicitudes.Add(sol);
            }
        ...
        if (solicitudes.Count > 0)
        {
            ...
            foreach (var item in dataSheets)
            {
                ...
                localReportDS.AddDataSource("DataSet1", list); //debere recibir una lista
