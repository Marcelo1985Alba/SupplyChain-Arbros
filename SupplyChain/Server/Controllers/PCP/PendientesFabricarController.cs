...
public class PendientesFabricarController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<List<vPendienteFabricar>>> Get()
    {
        try
        {
            ...
            //ACTUALIZA REGISTROS: SE COMENTA PORQUE VUELVE A ACTUALIZAR LA CANTIDAD A EMITIR
            //await _context.Database.ExecuteSqlRawAsync("EXEC NET_PCP_Pedidos");
            ...
            var xlista = await _context.VPendientesFabricars.ToListAsync();
            return xlista;
        }
        catch (Exception ex)
        {
    ...
    public async Task<ActionResult<List<vPendienteFabricar>>> PutPenFab(int id, ModeloPendientesFabricar prev)
    {
        if (id != prev.REGISTRO) return BadRequest();
        var xCg_form = prev.CG_FORM.ToString();
        if (xCg_form == "0")
            return BadRequest();
        //string xRegistro = prev.REGISTRO.ToString().Replace(",", ".").ToString();
    ...
    public List<vPendienteFabricar> EmitirOrdenes()
    {
        try
        {
            ...
            var xSQLCommandString =
                "SELECT REGISTRO, PEDIDO, CG_ART, DES_ART, PREVISION, CANTPED, CALCULADO, CANTEMITIR, LOPTIMO, STOCK, STOCKMIN, ";
            xSQLCommandString +=
                "ISNULL((SELECT TOP 1 CG_FORM FROM FORM2 WHERE ACTIVO != 'N' AND CG_PROD = A.CG_ART), 0) AS CG_FORM, ";
            xSQLCommandString +=
                "(case when (SELECT SUM(CANT) FROM Programa B WHERE A.CG_ART=B.CG_PROD AND CG_ESTADOCARGA=1) IS NULL then 0 else (SELECT SUM(CANT) FROM Programa B WHERE A.CG_ART=B.CG_PROD AND CG_ESTADOCARGA=1) end) AS STOCKENT, ";
            xSQLCommandString +=
                "(case when (SELECT SUM(CANT) FROM Programa B WHERE A.CG_ART=B.CG_PROD AND CG_ESTADOCARGA=0) IS NULL then 0 else (SELECT SUM(CANT) FROM Programa B WHERE A.CG_ART=B.CG_PROD AND CG_ESTADOCARGA=0) end) AS COMP_EMITIDAS, ";
            xSQLCommandString += "(CASE WHEN A.EXIGEOA = 1 THEN 'Armado' ELSE 'Fabricación' END) AS EXIGEOA ";
            xSQLCommandString += "FROM Net_temp_pedidos A ORDER BY CG_ART";
            dbPendFab = xConexionSQL.EjecutarSQL(xSQLCommandString);

            var xLista = dbPendFab.AsEnumerable().Select(m => new vPendienteFabricar
            {
                REGISTRO = m.Field<decimal>("REGISTRO"),
                PEDIDO = m.Field<decimal>("PEDIDO"),
                EXIGEOA = m.Field<string>("EXIGEOA"),
        ...
        {
            return new List<vPendienteFabricar>();
        }
    }
}