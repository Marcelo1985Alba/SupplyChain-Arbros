...
public class PlanificacionController : ControllerBase
{
    ...
    {
        _context = context;
        _stockCorregidoRepository = stockCorregidoRepository;
    }
    ...
    public async Task<List<Planificacion>> GetAsync(int armado, int emitidas)
    {
        try
        {
            var xSQLCommandString = "SELECT A.SEM_ORIGEN, A.SEM_ABAST_PURO, A.SEM_ABAST, B.CG_ORDEN, A.CG_ORDF" +
    ...
    public async Task<ActionResult<List<DespiecePlanificacion>>> Despiece(string cg_prod, int formula, decimal cantidad)
    {
        try
        {
            ...
            dbPlanificacion = xConexionSQL
                .EjecutarSQL(
                    string.Format("EXEC NET_PCP_Despiece_Producto '{0}', {1}, {2}", cg_prod, formula, cantidad));

            var xConexionSQL2 = new ConexionSQL(CadenaConexionSQL);
            ...
            var cotizaciones = dbCotizaciones.AsEnumerable().Select(m => new Cotizaciones
            {
                Id = m.Field<int>("ID"),
            ...
            var xLista = dbPlanificacion.AsEnumerable()
            ...
            foreach (var item in xLista)
            {
                ...
                if (stock == null)
                    item.StockCorregido = new StockCorregido();
                else
                ...
                try
                {
                    ...
                    {
                        var xConexionSQL3 = new ConexionSQL(CadenaConexionSQL);
                        var xSQLCommandString2 = "SELECT * FROM FORM2 WHERE CG_PROD ='" + codigoInsumo +
                                                 "' and REVISION  = (SELECT MAX(REVISION) FROM FORM2 " +
                                                 "WHERE CG_PROD = '" + codigoInsumo + "')";

                        dbPlanificacion = xConexionSQL3.EjecutarSQL(xSQLCommandString2);
                        item.formulasSemielaborado = dbPlanificacion.AsEnumerable().Select(m => new Formula
                        {
                            Cg_Mat = m.Field<string>("CG_MAT"),
                            CANT_MAT = m.Field<decimal>("CANT_MAT"),
                            Cg_Prod = m.Field<string>("CG_PROD"),
                        ...
                        foreach (var mat in item.formulasSemielaborado)
                            if (!string.IsNullOrWhiteSpace(mat.Cg_Mat))
                            {
                                //solo ordenes de compra
                                var aux = await _context.Compras
                                        .Where(s => s.CG_MAT.Trim() == mat.Cg_Mat.Trim() && s.NUMERO > 0)
                                ...
                                if (aux != null)
                                {
                                    ...
                                    {
                                        var cot = cotizaciones.Where(s => s.FEC_ULT_ACT <= aux.FE_EMIT)
                                            .MaxBy(s => s.FEC_ULT_ACT).COTIZACION;
                    ...
                    {
                        var aux = await _context.Compras
                            .Where(s => s.CG_MAT.Trim() == item.CG_MAT.Trim() && s.NUMERO > 0)
                            .OrderByDescending(s => s.FE_EMIT).FirstOrDefaultAsync();
                        if (aux != null)
                        {
                            if (aux.MONEDA.Trim().ToLower() == "dolares")
    ...
    public List<FormulaPlanificacion> Formula(string cg_prod)
    {
        try
        {
            ...
            var xSQLCommandString =
                "SELECT CG_FORM, IIF(ACTIVO != 'N',STR(CG_FORM) + ' - ACTIVA',STR(CG_FORM)) AS DES_FORM, ACTIVO, MAX(REVISION) AS REVISION " +
    ...
    {
        var xConexionSQL = new ConexionSQL(CadenaConexionSQL);
        var xSQLcommandString = "UPDATE Programa SET Cg_form = " + cg_form + " WHERE Cg_ordf = " + pl.CG_ORDF;
    ...
    public async Task<List<Planificacion>> OrdenesCerradasYAnuladas(int Busqueda)
    {
        try
        {
            var xSQLCommandString = "SELECT TOP " + Busqueda +
                                    " A.SEM_ORIGEN, A.SEM_ABAST_PURO, A.SEM_ABAST, B.CG_ORDEN, A.CG_ORDF" +
                                    ", (CASE WHEN B.CG_ORDEN=1 THEN 'Producto' ELSE (CASE WHEN B.CG_ORDEN=2 " +
                                    "THEN 'Semi-Elaborado de Proceso' ELSE (CASE WHEN B.CG_ORDEN=3 THEN 'Semi-Elaborado' " +
                                    "ELSE (CASE WHEN B.CG_ORDEN=4 THEN 'Materia Prima' ELSE (CASE WHEN B.CG_ORDEN=10 THEN 'Insumo No Productivo / Articulo de Reventa' ELSE (CASE WHEN B.CG_ORDEN=11 THEN 'Herramental e Insumos Inventariables' ELSE (CASE WHEN B.CG_ORDEN=12 THEN 'Repuestos' ELSE (CASE WHEN B.CG_ORDEN=13 THEN 'Servicios' ELSE '' END) END) END) END) END) END) END) END) AS CLASE" +
                                    ", (CASE WHEN A.CG_R='' THEN 'Fabricación' ELSE (CASE WHEN A.CG_R='R' THEN 'Reproceso' ELSE (CASE WHEN A.CG_R='T' THEN 'Retrabajo' ELSE (CASE WHEN A.CG_R='S' THEN 'Seleccion' ELSE (CASE WHEN A.CG_R='A' THEN 'Armado' ELSE '' END) END) END) END) END) AS CG_R" +
    ...
    public async Task<ActionResult<Planificacion>> RehabilitarOrden(Planificacion pl)
    {
        ...
        if (ordfab is not null)
        {
            ...
            if (pl.CG_ESTADOCARGA < 4) ordfab.FE_CIERRE = null;
    ...
    {
        if (tipo.ToLower() == "vacio") tipo = string.Empty;
        try
        {
            if (cgform == 0) cgform = 1;

            var formatoFechaCorta = CultureInfo.CurrentCulture.DateTimeFormat.ShortDatePattern;
            var usuario = HttpContext.User.Identity.Name;
            var fechaEntrega = feEntrega.ToLocalTime().Date.ToString("yyyy-MM-dd");

            var query =
                "EXEC NET_PCP_EmitirOrden @tipo, @cgArt, @cantEmitir, @fechaEntrega, @cgform, @cgEstadoCarga, @semOrigen, @pedido, @usuario, @idGenerado OUTPUT";
            ...
            var idGeneradoParam = new SqlParameter("@IdGenerado", SqlDbType.Int)
