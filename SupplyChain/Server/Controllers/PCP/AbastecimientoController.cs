...
public class AbastecimientoController : ControllerBase
{
    ...
    public object AbastecimientoSyncf()
    {
        try
        {
            ...
            var xSQLCommandString = "SELECT * FROM NET_Temp_Abastecimiento";
            dbAbastecimientoMP = xConexionSQL.EjecutarSQL(xSQLCommandString);

            var xLista = dbAbastecimientoMP.AsEnumerable().Select(m =>
                new ModeloAbastecimiento
                {
            ...
            if (queryString.Keys.Contains("$filter"))
            {
                ...
                var filter =
                    string.Join("",
                        queryString["$filter"].ToString().Split(' ').Skip(2)); // get filter from querystring
    ...
    public List<ModeloAbastecimiento> AbastecimientoMP()
    {
        try
        {
            ...
            var xSQLCommandString =
                "SELECT CG_MAT, DES_MAT, CALCULADO, ACOMPRAR, ACOMPRAR_INFORMADO, STOCK, UNIDMED, UNIDCOMER, STOCK_MINIMO, PEND_SIN_OC, COMP_DE_ENTRADA, COMP_DE_SALIDA" +
    ...
    public List<ModeloAbastecimiento> AbastecimientoSEX()
    {
        try
        {
            ...
            xConexionSQL = new ConexionSQL(CadenaConexionSQL);
            var xSQLCommandString =
                "SELECT CG_MAT, DES_MAT, CALCULADO, ACOMPRAR, STOCK, UNIDMED, UNIDCOMER, STOCK_MINIMO, PEND_SIN_OC, COMP_DE_ENTRADA, COMP_DE_SALIDA" +
                ", STOCK_CORREG, EN_PROCESO, REQUERIDO, ENTRPREV, * FROM NET_Temp_Abastecimiento WHERE CG_ORDEN = 3";
            dbAbastecimientoSE = xConexionSQL.EjecutarSQL(xSQLCommandString);

            var xLista = dbAbastecimientoSE.AsEnumerable().Select(m =>
                new ModeloAbastecimiento
                {
                    CG_PROD = m.Field<string>("CG_PROD"),
                    CG_MAT = m.Field<string>("CG_MAT"),
    ...
    public async Task<ActionResult<ModeloAbastecimiento>> PutAbMP(ModeloAbastecimiento Ab)
    {
        ...
        try
        {
            //var ab = await _context.ModeloAbastecimiento.FirstOrDefaultAsync(m => m.CG_MAT == Ab.CG_MAT);
